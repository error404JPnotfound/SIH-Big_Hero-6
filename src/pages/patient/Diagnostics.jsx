  ...
  return (
    <AppLayout role="patient">
      <div className="p-4 md:p-6 space-y-6 max-w-3xl mx-auto">
        <div>
          <h1 className="text-2xl font-bold text-navy">My Diagnostics</h1>
          <p className="text-muted text-sm">Track your test requests and results</p>
        </div>
        ...
        <div className="space-y-4">
          {MOCK_DIAGNOSTICS.map(dx => {
            ...
            return (
              <div key={dx.id} className="bg-surface rounded-xl border border-border p-5 hover:shadow-sm transition-shadow">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-semibold text-navy text-sm">{dx.name}</h3>
                      <Badge variant={meta.variant}>{meta.label}</Badge>
                    </div>
                    <p className="text-xs text-muted mt-0.5">Requested by {dx.requested_by}</p>
                    ...
                    {dx.result && (
                      <div className="mt-3 flex items-center gap-2">
                        <span className="text-xs font-medium text-success">Result: {dx.result}</span>
