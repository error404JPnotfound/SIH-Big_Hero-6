  ...
  return (
    <div className="bg-surface rounded-2xl border border-border overflow-hidden shadow-sm">
      <div className="px-5 pt-5 pb-4 flex items-start gap-3 border-b border-border">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="font-semibold text-navy text-sm">{r.dept} — {r.to}</h3>
          ...
          <p className="text-xs text-muted mt-1 flex items-center gap-1"><Calendar className="w-3 h-3" />Referred on {new Date(r.date).toLocaleDateString('en-IN')}</p>
        </div>
      </div>
          ...
          {STATUS_STEPS.map((s, i) => (
            <div key={s} className="flex items-center flex-1">
              <div className={`w-full flex flex-col items-center gap-1 ${i <= stepIdx ? 'opacity-100' : 'opacity-30'}`}>
                <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${i < stepIdx ? 'bg-success text-white' : i === stepIdx ? 'bg-teal text-white' : 'bg-border text-muted'}`}>
                  {i < stepIdx ? '✓' : i + 1}
                </div>
                <span className="text-center text-xs text-muted leading-tight hidden sm:block">{s}</span>
              </div>
              {i < STATUS_STEPS.length - 1 && <div className={`flex-1 h-0.5 mx-1 ${i < stepIdx ? 'bg-success' : 'bg-border'}`} />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-5 pb-4">
        <p className="text-xs font-medium text-muted">Referring Doctor: {r.doctor}</p>
        {r.status === 'pending' && (
    ...
    <AppLayout role="patient">
      <div className="p-4 md:p-6 space-y-6 max-w-3xl mx-auto">
        <div>
          ...
          <p className="text-muted text-sm">Track your referral journey from PHC to Specialist</p>
        </div>

        {/* Referral path info */}
        <div className="bg-navy rounded-2xl p-5 text-surface text-sm">
          <p className="font-semibold mb-3 text-teal">Referral Pathway</p>
          <div className="flex items-center gap-2 flex-wrap text-surface/70 text-xs">
            {['Sub-Centre', 'PHC', 'Rural Hospital', 'District Hospital', 'Specialist'].map((s, i, arr) => (
              <span key={s} className="flex items-center gap-2">
                ...
                {i < arr.length - 1 && <ArrowRight className="w-3 h-3 text-teal" />}
              </span>
            ))}
          </div>
        </div>
