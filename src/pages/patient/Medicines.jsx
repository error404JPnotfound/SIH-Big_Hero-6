  ...
  return (
    <AppLayout role="patient">
      <div className="p-4 md:p-6 space-y-6 max-w-3xl mx-auto">
        <div>
          <h1 className="text-2xl font-bold text-navy">Medicine Availability</h1>
          <p className="text-muted text-sm">Check real-time medicine stock at nearby facilities</p>
        </div>
        ...
        <div className="relative">
          ...
          <input
            className="w-full h-11 pl-10 pr-4 rounded-xl border border-border bg-surface text-sm focus:outline-none focus:ring-2 focus:ring-teal"
        ...
        <div className="flex gap-2 flex-wrap">
          {[{ id: 'all', label: 'All' }, { id: 'available', label: 'Available Now' }, { id: 'govt', label: 'Govt Facility' }].map(f => (
            <button key={f.id} onClick={() => setFilter(f.id)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${filter === f.id ? 'bg-teal text-white border-teal' : 'border-border text-muted hover:border-teal hover:text-teal'}`}>
              {f.label}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-3">
          {filtered.map(m => (
            <div key={m.id} className="bg-surface rounded-xl border border-border p-4 hover:shadow-sm transition-shadow">
              <div className="flex items-start gap-3">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${m.available ? 'bg-success-bg' : 'bg-critical-bg'}`}>
                  <Pill className={`w-5 h-5 ${m.available ? 'text-success' : 'text-critical'}`} />
                </div>
